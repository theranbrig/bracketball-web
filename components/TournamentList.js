...
const TournamentList = ({ user }) => {
  ...
  return (
    <>
      <div className='w-full grid grid-cols-3 grid-rows-2 gap-4 mb-4 justify-center items-center px-4'>
        {!firebaseLoading
          ? myTournaments
      ...
      <div className='flex flex-row justify-between items-center w-full px-4'>
        ...
        <div>
          <button aria-label="show fewer pools" disabled={currentPage === 1} onClick={() => handleChange(-1)}>
            <BsChevronLeft color='#1d3557' size='2rem' />
          </button>
          <button aria-label="show more pools" disabled={currentPage === totalPages} onClick={() => handleChange(1)}>
