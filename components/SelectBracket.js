          ...
          return (
            <div className=''>
              <div className={`flex flex-row p-2 ${idx % 2 !== 0 ? 'bg-celadon' : 'bg-powder'}`}>
                <h4 className='group-title bg-honeydew px-6 py-1 text-prussian rounded-md text-sm border-2 border-prussian'>{group.group}</h4>
                <ul className='w-full flex flex-col justify-between'>
                  {group.groupTeams.map((team) => {
                    return (
                      <button className='bg-prussian text-honeydew p-1 rounded-full pl-4 my-2  border-2 border-honeydew text-left hover:bg-prussianDark hover:shadow-lg shadow-honeydew'>
